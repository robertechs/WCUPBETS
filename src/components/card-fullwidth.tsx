...
export default function CardFullWidth({
    ...
    <div className="wc-stats-grid">
      ...
      <div className="wc-stat wc-stat-gold">
        ...
        <span className="wc-stat-mono">
          {marketType === "h2h" && priceB ? `${priceA} / ${priceB}` : priceA}
        </span>
      </div>
    </div>
  );
    ...
    <div className="flex flex-col w-full text-[#0f172a] font-semibold mt-2">
      <div className="flex justify-between">
        <div
          ...
          style={{ background: "linear-gradient(to right, rgba(34,197,94,0.12), transparent)" }}
        >
          <span className="text-[13px]" style={{ color: GREEN }}>{percentage}</span>
          <span className="text-[13px]">{t("home.chance")}</span>
        </div>
        <div
          className="h-[34px] flex items-center gap-1 px-3 rounded-lg"
          style={{ background: "linear-gradient(to right, transparent, rgba(181,46,41,0.08))" }}
        >
      ...
      <div className="flex gap-2 mt-2">
        ...
        >
          <span className="text-[#0f172a] font-semibold text-[18px]">{t("home.yes")}</span>
        </button>
        <button
          type="button"
          onClick={handleNo}
  ...
  const bigVisual =
    marketType === "h2h" ? (
      <div className="flex items-center justify-center gap-3 h-[160px] w-full max-w-[200px]">
        <TokenIcon symbol={tokens[0]?.symbol ?? ""} countryCode={tokens[0]?.countryCode} flag={tokens[0]?.flag ?? ""} size={64} />
        <span className="text-[#052e16] text-xl tracking-wide" style={{ fontFamily: "var(--font-bebas), sans-serif" }}>
          vs
        </span>
        <TokenIcon symbol={tokens[1]?.symbol ?? ""} countryCode={tokens[1]?.countryCode} flag={tokens[1]?.flag ?? ""} size={64} />
      </div>
    ) : (
      <div className="flex items-center justify-center h-[160px] w-[160px]">
        <TokenIcon symbol={tokens[0]?.symbol ?? ""} countryCode={tokens[0]?.countryCode} flag={tokens[0]?.flag ?? ""} size={120} />
      </div>
    );

  return (
    <>
      <div className="wc-card md:px-6 md:pt-6 md:pb-5">
          ...
          <div className="flex justify-between mt-3 gap-2 text-[12px] text-[rgba(51,65,85,0.85)]">
            <div className="flex gap-2 items-center min-w-0">
              ...
              <span className="truncate">
                <span className="font-semibold text-[#0f172a]">{t("home.ends")}</span>{" "}
          ...
          <div className="flex gap-8">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              <div className="flex flex-wrap items-start justify-between gap-3 w-full">
                <h2 className="wc-card-title max-w-full md:max-w-[min(100%,42rem)]">{title}</h2>
                <span className="text-sm text-[rgba(71,85,105,0.92)] shrink-0 max-w-[38%] text-right leading-snug">
                  {topic}
                </span>
              </div>
              {statsBlock}
